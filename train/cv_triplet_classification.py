...
def classification_train_process(dataset, model, k_fold=5, batch_size=64, test_dataset=None):
  ...
  for fold in range(k_fold):
    ...
    for epoch in range(epochs):
      ...
      print(f'Fold {fold + 1}: Average loss: {avg_loss}, Average validation loss: {avg_val_loss}')
    ...
    if test_dataset is not None:
      avg_test_loss, accuracy = validate_model(model, test_dataset, batch_size=batch_size)
      print(f'Fold {fold + 1}: Average test loss: {avg_test_loss}, Accuracy: {accuracy:.2f}%')
  ...
  print(f'Over all folds: Average loss : {average_loss}, Average validation loss: {average_val_loss}')
  ...
  if not result_df.empty:
    ...
    result_df.to_csv(
        f'results/cv-triplet/{current_dataset}_{current_backbone_model._get_name()}_{time.strftime("%Y%m%d-%H%M%S")}.csv', index=False)
...
def create_training_process_df(
  ...
  create_backbone_model_funcs = [
      (lambda: torchvision.models.resnet50(weights=None)),  # ResNet50
      (lambda: torchvision.models.vgg16(weights=None)),  # VGG16
      (lambda: torchvision.models.mobilenet_v2(weights=None)),  # MobileNetV2
      # Uncomment the following line to add DenseNet121 if needed
      # (lambda: torchvision.models.densenet121(weights=None)), # DenseNet121
  ]
...
if __name__ == '__main__':
  ...
  for index, row in triplet_df.iterrows():
    ...
    create_classification_test_dataset_fn = row.get('create_classification_test_dataset_fn', None)
    ...
    avg_test_loss, accuracy = validate_model(trained_model, test_ds, batch_size=batch_size)
    print(
    ...
    train_ds = ds.EmbeddedDataset(train_ds, trained_model, is_moving_labels_to_function=False)
    test_ds = ds.EmbeddedDataset(test_ds, trained_model, is_moving_labels_to_function=False)
    print(
    ...
    # Validate the model on the test set
    avg_test_loss, accuracy = validate_model(trained_classifier_model, test_ds, batch_size=batch_size)
    print(
        f'Average loss: {avg_loss}, Average val loss: {avg_val_loss}, Average test loss: {avg_test_loss}, Accuracy: {accuracy:.2f}%')
    ...
    train_ds = ds.EmbeddedDataset(train_ds, trained_model, is_moving_labels_to_function=True)
    test_ds = ds.EmbeddedDataset(test_ds, trained_model, is_moving_labels_to_function=True)
    train_ds.apply_function_to_labels_embeddings(lambda x: x)  # Example transformation, can be customized
    test_ds.apply_function_to_labels_embeddings(lambda x: x)  # Example transformation, can be customized
    print(
        f'Training classification model {classifier_model._get_name()} on dataset {classifier_dataset.__class__.__name__}...')
    trained_classifier_model, avg_loss, avg_val_loss = train(
        train_ds, classifier_model, train_process='classification', k_fold=5, batch_size=batch_size, test_dataset=test_ds)
    ...
    train_ds = ds.EmbeddedDataset(train_ds, trained_model, is_moving_labels_to_function=True)
    test_ds = ds.EmbeddedDataset(test_ds, trained_model, is_moving_labels_to_function=True)
    train_ds.apply_function_to_labels_embeddings(lambda x: 4 * x)  # Example transformation, can be customized
    test_ds.apply_function_to_labels_embeddings(lambda x: 4 * x)  # Example transformation, can be customized
